...
Class BonfireButton
    ...
    Protected Overrides Sub OnPaint(ByVal e As System.Windows.Forms.PaintEventArgs)
        ...
        Else
            Dim textSize As New Size(G.MeasureString(Text, Font).Width, G.MeasureString(Text, Font).Height)
...
Class BonfireCombo : Inherits ComboBox
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        Dim BorderColor As New Pen(Color.FromArgb(43, 169, 232))
        If Enabled = False Then BorderColor = New Pen(Color.DarkGray)

        Dim mainRect As New Rectangle(0, 0, Width - 1, Height - 1)
        G.FillRectangle(New SolidBrush(Color.FromArgb(240, 240, 240)), mainRect)
        G.DrawRectangle(BorderColor, mainRect)

        Dim triangleY As Integer = (Height - 5) \ 2
        Dim triangle As Point() = New Point() {New Point(Width - 14, triangleY + 6), New Point(Width - 18, triangleY), New Point(Width - 10, triangleY)}
        Dim TriangleColor As New SolidBrush(Color.FromArgb(43, 169, 232))
        If Enabled = False Then TriangleColor = New SolidBrush(Color.DarkGray)
        G.FillPolygon(TriangleColor, triangle)
        G.DrawLine(BorderColor, New Point(Width - 27, 0), New Point(Width - 27, Height - 1))

        Try
            If Items.Count > 0 Then
                Dim text As String = If(SelectedIndex >= 0, Items(SelectedIndex), Items(0))
                Dim textX As Integer = -5 + (Width - 2 * 6 - G.MeasureString(text, Font).Width) \ 2
                Dim textY As Integer = ((Height - 1) / 2) - (G.MeasureString(text, Font).Height / 2) + 1

                G.DrawString(text, Font, Brushes.Gray, New Point(textX, textY))
            End If
        Catch : End Try
    End Sub

    Sub Item_DrawItem(sender As Object, e As DrawItemEventArgs) Handles Me.DrawItem
        e.DrawBackground()
        ...
        Try
            Dim text As String = GetItemText(Items(e.Index))
            Dim textRect As New Rectangle(e.Bounds.X + 6, e.Bounds.Y + 2, e.Bounds.Width - 6, e.Bounds.Height - 4)

            If (e.State And DrawItemState.Selected) = DrawItemState.Selected Then
                G.FillRectangle(New SolidBrush(Color.FromArgb(20, 160, 230)), e.Bounds)
                G.DrawString(text, Font, Brushes.White, textRect, New StringFormat() With {.Alignment = StringAlignment.Center, .LineAlignment = StringAlignment.Center})
                G.DrawRectangle(New Pen(Color.FromArgb(20, 160, 230)), e.Bounds)
            Else
                G.FillRectangle(Brushes.White, e.Bounds)
                G.DrawString(text, Font, Brushes.DarkGray, textRect, New StringFormat() With {.Alignment = StringAlignment.Center, .LineAlignment = StringAlignment.Center})
                G.DrawRectangle(New Pen(Color.FromArgb(20, 160, 230)), e.Bounds)
            End If
        Catch : End Try
    End Sub
