...
#define IS_MOD_KEY(vk) ((vk)==VK_LSHIFT || (vk)==VK_RSHIFT || (vk)==VK_SHIFT || (vk)==VK_LCONTROL || (vk)==VK_RCONTROL || (vk)==VK_CONTROL || (vk)==VK_LMENU || (vk)==VK_RMENU || (vk)==VK_MENU || (vk)==VK_LWIN || (vk)==VK_RWIN)
...
static const char* ToUnicodeVkToUtf8(DWORD vk, char* out, size_t outCap) {
    ...
    if (r >= 1) {
        ...
        if (n > 0) { 
            ...
            if (c0 >= 0x20 && c0 != 0x7F) { // hex for ' ' (space) and DEL
                out[n] = '\0';
                return (out);
            } 
        }
